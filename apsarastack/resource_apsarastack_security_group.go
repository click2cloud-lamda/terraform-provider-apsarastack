...
func resourceApsaraStackSecurityGroupCreate(d *schema.ResourceData, meta interface{}) error {
	...
	//request.SecurityGroupType = d.Get("security_group_type").(string)//has been removed for Apsarastack

	if v := d.Get("vpc_id").(string); v != "" {
		request.VpcId = v
	}

	//request.ResourceGroupId = d.Get("resource_group_id").(string)//has been removed for Apsarastack
...
func resourceApsaraStackSecurityGroupRead(d *schema.ResourceData, meta interface{}) error {
	...
	//d.Set("inner_access", object.InnerAccessPolicy == string(GroupInnerAccept))//has been deprecated for Apsarastack
	...
	//d.Set("security_group_type", response.SecurityGroups.SecurityGroup[0].SecurityGroupType)//has been removed for Apsarastack
...
func resourceApsaraStackSecurityGroupUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if d.HasChange("inner_access_policy") /*|| d.HasChange("inner_access") || d.IsNewResource() && d.Get("security_group_type").(string) != "enterprise" */ {
		...
		} // else if v, ok := d.GetOkExists("inner_access"); ok && !v.(bool) {//has been deprecated for Apsarastack
